            ...
            <div class="custom-products-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                ...
                if (have_posts()) :
                    while (have_posts()) : the_post();
                        ...
                        ?>
                        <div class="custom-product-item p-8 bg-white rounded-2xl relative" onclick="window.location.href='<?php the_permalink(); ?>'" style="cursor: pointer;">
                <div class="product-thumbnail-wrapper relative">
                    ...
                    <a href="<?php the_permalink(); ?>" class="product-link">
                        ...
                        <div class="product-price custom-price-display">
                            <?php
                            $regular_price = $product->get_regular_price();
                            $sale_price = $product->get_sale_price();
...
jQuery(document).ready(function($) {
    ...
    let lastAutoSelectedCategory = null;

    // ახალი ფუნქცია URL-ის შესამოწმებლად
    function checkSpecificUrlCategories() {
        const urlParams = new URLSearchParams(window.location.search);
        const selectedCategories = urlParams.getAll('product_categories[]');
        
        // თუ URL-ში გვაქვს 28 და 38
        if (selectedCategories.includes('28') && selectedCategories.includes('38')) {
            isAutoSelecting = true;
            
            // ავტომატურად მონიშვნა
            $('.category-filter-checkbox').each(function() {
                const value = $(this).val();
                if (value === '28' || value === '38') {
    ...
    // ავტომატური მონიშვნის ფუნქცია
    function handleAutoSelection(changedCheckbox) {
        if (isAutoSelecting) return;
        
        const $checkbox = $(changedCheckbox);
        const $categoryItem = $checkbox.closest('.category-item');
        const categoryName = $categoryItem.find('.category-name').text().trim();
        const isSubcategory = $categoryItem.closest('.subcategories').length > 0;
        
        console.log('Changed category:', categoryName, 'Is subcategory:', isSubcategory);
        
        // თუ "საკვები დანამატი / ვიტამინები" შეიცვალა
        if (categoryName === 'საკვები დანამატი / ვიტამინები' && isSubcategory) {
            isAutoSelecting = true;
            
            if ($checkbox.is(':checked')) {
                console.log('საკვები დანამატი / ვიტამინები checked - auto selecting Kohler Pharma');
                // ვეძებთ და ვნიშნავთ Kohler Pharma-ს
                $('.category-item').each(function() {
                    const $mainCategory = $(this);
                    const mainCategoryName = $mainCategory.find('.category-name').text().trim();
                    if (mainCategoryName === 'Kohler Pharma') {
                        $mainCategory.find('.category-filter-checkbox').prop('checked', true);
    ...
    // განახლებული მშობელი კატეგორიების გახსნის ფუნქცია
    function openParentCategories() {
        // ვხსნით current-cat კლასის მქონე კატეგორიების მშობლებს
        ...
        // ვხსნით მონიშნული კატეგორიების მშობლებს
        $('.category-filter-checkbox:checked').each(function() {
            const $checkbox = $(this);
            const $subcategories = $checkbox.closest('.subcategories');
            
            if ($subcategories.length > 0) {
                $subcategories.each(function() {
                    const $subcat = $(this);
                    // ვხსნით მშობელ subcategories
                    $subcat.addClass('active').show();
                    // ვააქტიურებთ toggle ღილაკს
                    $subcat.siblings('.category-header').find('.toggle-subcategories').addClass('active');
                    // ვხსნით parent subcategories-საც (თუ არსებობს)
    ...
    // URL-იდან კატეგორიების ��ონიშვნის ფუნქცია
    function checkCategoriesFromUrl() {
        ...
        if (selectedCategories.length > 0) {
            selectedCategories.forEach(categoryId => {
                $('.category-filter-checkbox[value="' + categoryId + '"]').prop('checked', true);
            });
        }
    }

    // Event Listeners
    $(document).on('change', '.category-filter-checkbox', function(e) {
        handleAutoSelection(this);
        filterProducts(1);
    });
    ...
    // 1. ვამოწმებთ სპეციფიურ URL კატეგორიებს
    checkSpecificUrlCategories();
    // 2. ვნიშნავთ URL-ში არსებულ კატეგორიებს
    checkCategoriesFromUrl();
    // 3. ვხსნით შესაბამის დროფდაუნებს
    openParentCategories();
    // 4. ვამუშავებთ ავტომატურ მონიშვნებს
    handleAutoSelection();
});
</script>
